...
//        chartView.legend = l
    ...
    func setDataCount(_ count: Int, range: UInt32) {
        ...
        let set1 = BarChartDataSet(entries: yVals1, label: "Company A", xAxisAccessibilityLabel: (0 ..< yVals1.count).map { "X-axis-\($0)" }, yAxisAccessibilityLabel: (0 ..< yVals1.count).map { "Y-axis-\($0)" })
        set1.setColor(UIColor(red: 104/255, green: 241/255, blue: 175/255, alpha: 1))
        
        let set2 = BarChartDataSet(entries: yVals2, label: "Company B", xAxisAccessibilityLabel: (0 ..< yVals2.count).map { "X-axis-\($0)" }, yAxisAccessibilityLabel: (0 ..< yVals2.count).map { "Y-axis-\($0)" })
        set2.setColor(UIColor(red: 164/255, green: 228/255, blue: 251/255, alpha: 1))
        
        let set3 = BarChartDataSet(entries: yVals3, label: "Company C", xAxisAccessibilityLabel: (0 ..< yVals3.count).map { "X-axis-\($0)" }, yAxisAccessibilityLabel: (0 ..< yVals3.count).map { "Y-axis-\($0)" })
        set3.setColor(UIColor(red: 242/255, green: 247/255, blue: 158/255, alpha: 1))
        
        let set4 = BarChartDataSet(entries: yVals4, label: "Company D", xAxisAccessibilityLabel: (0 ..< yVals4.count).map { "X-axis-\($0)" }, yAxisAccessibilityLabel: (0 ..< yVals4.count).map { "Y-axis-\($0)" })
